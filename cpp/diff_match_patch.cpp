...
QList< Diff > diff_match_patch::diff_compute( QString text1, QString text2, bool checklines, clock_t deadline )
{
    QList< Diff > diffs;
...
QList< Diff > diff_match_patch::diff_bisect( const QString &text1, const QString &text2, clock_t deadline )
{
...
QString diff_match_patch::diff_linesToCharsMunge( const QString &text, QStringList &lineArray, QMap< QString, int > &lineHash )
{
    int lineStart = 0;
    int lineEnd = -1;
    QString line;
...
void diff_match_patch::diff_cleanupSemantic( QList< Diff > &diffs )
{
    ...
    while ( thisDiff != NULL )
    {
        ...
        else
        {
            ...
            // sides of it.
            if ( !lastequality.isNull() && ( lastequality.length() <= std::max( length_insertions1, length_deletions1 ) ) && ( lastequality.length() <= std::max( length_insertions2, length_deletions2 ) ) )
            {
            ...
            case EQUAL:
                if ( count_delete + count_insert > 1 )
                {
                    ...
                    if ( both_types )
                    {
                        ...
                        {
                            thisDiff = &pointer.next();
                            thisDiff->text = safeMid( text_insert, text_insert.length() - commonlength ) + thisDiff->text;
    ...
    while ( nextDiff != NULL )
    {
        if ( prevDiff->operation == EQUAL && nextDiff->operation == EQUAL )
        {
            ...
            if ( thisDiff->text.endsWith( prevDiff->text ) )
            {
                ...
                thisDiff->text = prevDiff->text + thisDiff->text.left( thisDiff->text.length() - prevDiff->text.length() );
...
void diff_match_patch::patch_addContext( Patch &patch, const QString &text )
{
    ...
    while ( text.indexOf( pattern ) != text.lastIndexOf( pattern ) && pattern.length() < Match_MaxBits - Patch_Margin - Patch_Margin )
    {
        ...
        pattern = safeMid( text, std::max( 0, patch.start2 - padding ), std::min( text.length(), patch.start2 + patch.length1 + padding ) - std::max( 0, patch.start2 - padding ) );
    }
    // Add one chunk for good luck.
    padding += Patch_Margin;

    // Add the prefix.
    QString prefix = safeMid( text, std::max( 0, patch.start2 - padding ), patch.start2 - std::max( 0, patch.start2 - padding ) );
    if ( !prefix.isEmpty() )
    {
        patch.diffs.prepend( Diff( EQUAL, prefix ) );
    }
    // Add the suffix.
    QString suffix = safeMid( text, patch.start2 + patch.length1, std::min( text.length(), patch.start2 + patch.length1 + padding ) - ( patch.start2 + patch.length1 ) );
    if ( !suffix.isEmpty() )
    {
    ...
    foreach( Diff aDiff, diffs )
    {
        ...
        switch ( aDiff.operation )
        {
            case INSERT:
                ...
                postpatch_text = postpatch_text.left( char_count2 ) + aDiff.text + safeMid( postpatch_text, char_count2 );
                break;
            case DELETE:
                patch.length1 += aDiff.text.length();
                patch.diffs.append( aDiff );
                postpatch_text = postpatch_text.left( char_count2 ) + safeMid( postpatch_text, char_count2 + aDiff.text.length() );
                break;
            case EQUAL:
                if ( aDiff.text.length() <= 2 * Patch_Margin && !patch.diffs.isEmpty() && !( aDiff == diffs.back() ) )
                {
    ...
    foreach( Patch aPatch, patchesCopy )
    {
        ...
        else
        {
            ...
            if ( text1 == text2 )
            {
                ...
                text = text.left( start_loc ) + diff_text2( aPatch.diffs ) + safeMid( text, start_loc + text1.length() );
            }
            else
            {
                ...
                else
                {
                    ...
                    foreach( Diff aDiff, aPatch.diffs )
                    {
                        if ( aDiff.operation != EQUAL )
                        {
                            ...
                            {
                                // Insertion
                                text = text.left( start_loc + index2 ) + aDiff.text + safeMid( text, start_loc + index2 );
                            }
                            else if ( aDiff.operation == DELETE )
                            {
                                // Deletion
                                text = text.left( start_loc + index2 ) + safeMid( text, start_loc + diff_xIndex( diffs, index1 + aDiff.text.length() ) );
                            }
                        }
                        if ( aDiff.operation != DELETE )
                        {
        ...
        while ( !bigpatch.diffs.isEmpty() )
        {
            ...
            while ( !bigpatch.diffs.isEmpty() && patch.length1 < patch_size - Patch_Margin )
            {
                ...
                else
                {
                    ...
                    diff_text = diff_text.left( std::min( diff_text.length(), patch_size - patch.length1 - Patch_Margin ) );
                    patch.length1 += diff_text.length();
                    start1 += diff_text.length();
                    if ( diff_type == EQUAL )
                    {
