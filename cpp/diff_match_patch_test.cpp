...
void diff_match_patch_test::assertEquals( const QString &strCase, const QString &s1, const QString &s2 )
{
    if ( s1 != s2 )
    {
        qDebug( "%s FAIL\nExpected: %s\nActual: %s", qPrintable( strCase ), qPrintable( s1 ), qPrintable( s2 ) );
        throw strCase;
    }
    qDebug( "%s OK", qPrintable( strCase ) );
}
...
void diff_match_patch_test::assertEquals( const QString &strCase, const Diff &d1, const Diff &d2 )
{
    if ( d1 != d2 )
    {
        qDebug( "%s FAIL\nExpected: %s\nActual: %s", qPrintable( strCase ), qPrintable( d1.toString() ), qPrintable( d2.toString() ) );
        throw strCase;
    }
    qDebug( "%s OK", qPrintable( strCase ) );
}
...
void diff_match_patch_test::assertEquals( const QString &strCase, const QList< Diff > &list1, const QList< Diff > &list2 )
{
    ...
    if ( fail )
    {
        ...
        listString2 += ")";
        qDebug( "%s FAIL\nExpected: %s\nActual: %s", qPrintable( strCase ), qPrintable( listString1 ), qPrintable( listString2 ) );
        throw strCase;
    }
    qDebug( "%s OK", qPrintable( strCase ) );
}
...
void diff_match_patch_test::assertEquals( const QString &strCase, const QVariant &var1, const QVariant &var2 )
{
    if ( var1 != var2 )
    {
        qDebug( "%s FAIL\nExpected: %s\nActual: %s", qPrintable( strCase ), qPrintable( var1.toString() ), qPrintable( var2.toString() ) );
        throw strCase;
    }
    qDebug( "%s OK", qPrintable( strCase ) );
}
...
void diff_match_patch_test::assertEquals( const QString &strCase, const QMap< QChar, int > &m1, const QMap< QChar, int > &m2 )
{
    ...
    while ( i1.hasNext() && i2.hasNext() )
    {
        ...
        {
            qDebug( "%s FAIL\nExpected: (%c, %d)\nActual: (%c, %d)", qPrintable( strCase ), i1.key().toAscii(), i1.value(), i2.key().toAscii(), i2.value() );
            throw strCase;
        }
    }
    ...
    {
        i1.next();
        qDebug( "%s FAIL\nExpected: (%c, %d)\nActual: none", qPrintable( strCase ), i1.key().toAscii(), i1.value() );
        throw strCase;
    }
    if ( i2.hasNext() )
    {
        i2.next();
        qDebug( "%s FAIL\nExpected: none\nActual: (%c, %d)", qPrintable( strCase ), i2.key().toAscii(), i2.value() );
        throw strCase;
    }
    qDebug( "%s OK", qPrintable( strCase ) );
}
...
void diff_match_patch_test::assertEquals( const QString &strCase, const QStringList &list1, const QStringList &list2 )
{
    if ( list1 != list2 )
    {
        qDebug( "%s FAIL\nExpected: %s\nActual: %s", qPrintable( strCase ), qPrintable( list1.join( "," ) ), qPrintable( list2.join( "," ) ) );
        throw strCase;
    }
    qDebug( "%s OK", qPrintable( strCase ) );
}
...
QList< Diff > diff_match_patch_test::diffList( Diff d1, Diff d2, Diff d3, Diff d4, Diff d5, Diff d6, Diff d7, Diff d8, Diff d9, Diff d10 )
{
